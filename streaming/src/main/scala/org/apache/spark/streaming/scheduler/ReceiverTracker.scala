 ...
 */
private[streaming] case class RestartReceiver(receiver: Receiver[_], receiverNum: Int)
 ...
 * at the first time.
 */
private[streaming] case class StartAllReceivers(receiver: Seq[Receiver[_]], receiverNum: Int)
  extends ReceiverTrackerLocalMessage
...
private[streaming] case class UpdateBroadcast(SplitKeyPar: mutable.HashMap[String, Integer]) extends ReceiverTrackerMessage
...
class ReceiverTracker(ssc: StreamingContext, skipReceiverLaunch: Boolean = false) extends Logging {
  ...
  private def launchReceivers(): Unit = {
    ...
    endpoint.send(StartAllReceivers(receivers, receiverInputStreams.size))
  }
  ...
  private def updateBroadcast(SplitKeyPar: mutable.HashMap[String, Integer]): Unit = {

    ssc.sparkContext.storeSplitKeyPar(SplitKeyPar)
    /*
    if (AstraeaBatchKeyInfo.keyParBC != null) {
      AstraeaBatchKeyInfo.keyParBC.unpersist()
      AstraeaBatchKeyInfo.keyParBC = ssc.sparkContext.broadcast(SplitKeyPar)
    } else {
      AstraeaBatchKeyInfo.keyParBC = ssc.sparkContext.broadcast(SplitKeyPar)
    }
    logInfo("---------Astraea info:--------- in ReceiverTracker, SplitKeyPar.size = " + SplitKeyPar.size)
    //logInfo("---------Astraea info:--------- in ReceiverTracker, broadcastKeyPartition.size = " + AstraeaBatchKeyInfo.keyParBC.value.size)
    logInfo("---------Astraea info:--------- in ReceiverTracker, getBroadcast.size = " + AstraeaBatchKeyInfo.getBroadcast.size)
  ...
  private class ReceiverTrackerEndpoint(override val rpcEnv: RpcEnv) extends ThreadSafeRpcEndpoint {
    ...
    //private val SplitKeyPar = mutable.HashMap[String, Integer]()

    override def receive: PartialFunction[Any, Unit] = {
      // Local messages
      case StartAllReceivers(receivers, receiverNum) =>
        val scheduledLocations = schedulingPolicy.scheduleReceivers(receivers, getExecutors)
        for (receiver <- receivers) {
          val executors = scheduledLocations(receiver.streamId)
          updateReceiverScheduledExecutors(receiver.streamId, executors)
          receiverPreferredLocations(receiver.streamId) = receiver.preferredLocation
          startReceiver(receiver, executors, receiverNum)
        }
      case RestartReceiver(receiver, receiverNum) =>
        ...
        // receiverTrackingInfos
        startReceiver(receiver, scheduledLocations, receiverNum)
      ...
      //case UpdateBroadcast(splitKeyPar) =>
      //  updateBroadcast(splitKeyPar)
    }
    ...
    override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
      ...
      case AddBlock(receivedBlockInfo) =>
        //接收supervisor里的trackerEndpoint.askWithRetry(AddBlock(blockInfo))
        //调用addBlock方法
      ...
      val startReceiverFunc: Iterator[Receiver[_]] => Unit =
        (iterator: Iterator[Receiver[_]]) => {
          ...
          if (TaskContext.get().attemptNumber() == 0) {
            //有几个receiver，就创建几个ReceiverSupervisorImpl
            val receiver = iterator.next()
            assert(iterator.hasNext == false)
            //2021.9.8 这里可以传入receiver的数量(numReceiver)，以控制当数量大于1时，把所有的key都设为split
            val supervisor = new ReceiverSupervisorImpl(
              //receiver, SparkEnv.get, serializableHadoopConf.value, checkpointDirOption)
              //receiver, SparkEnv.get, serializableHadoopConf.value, checkpointDirOption, receiverNum)
              receiver, SparkEnv.get, serializableHadoopConf.value, checkpointDirOption, receiverNum)
            supervisor.start()
      ...
      future.onComplete {
        case Success(_) =>
          if (!shouldStartReceiver) {
            ...
            self.send(RestartReceiver(receiver, receiverNum))
          }
        case Failure(e) =>
          if (!shouldStartReceiver) {
            onReceiverJobFinish(receiverId)
