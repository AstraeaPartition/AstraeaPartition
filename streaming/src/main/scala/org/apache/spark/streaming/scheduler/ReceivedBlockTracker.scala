...
private[streaming] class ReceivedBlockTracker(
  ...
  //调用本类的时候数据已经划分完block了，需要把block和batch信息对应起来
  //每个Receiver对应有一个streamId，这里就是给每一个Receiver配置了一个队列，来存储从该队列获取到的Block信息
  private val streamIdToUnallocatedBlockQueues = new mutable.HashMap[Int, ReceivedBlockQueue]
  //每个Block都是所属于某个batch的，这里就是记录每个batch有哪些block的
   ...
   * 这里在addBlock的时候只是把接收到的block数据加入到未处理的队列中，然后在下个批次开始的时候，会需要generateJobs
   */
   ...
   * 在JobGenerator中定时器定期执行generateJobs
  ...
  def allocateBlocksToBatch(batchTime: Time): Unit = synchronized {
    if (lastAllocatedBatchTime == null || batchTime > lastAllocatedBatchTime) {
      ...
      if (writeToLog(BatchAllocationEvent(batchTime, allocatedBlocks))) {
        //将所有的从streamIdToAllocatedBlocks中获取的数据一起作为value
        //放入到timeToAllocatedBlocks的hashMap中，从而指定了当前批次的block数据
