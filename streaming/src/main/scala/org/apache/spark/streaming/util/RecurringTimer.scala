...
private[streaming]
class RecurringTimer(clock: Clock, period: Long, callback: (Long) => Unit, name: String, blockInterval: Long = 100)
  extends Logging {
  ...
  def getAstraeaTime(): Long = {
    //可以在这个地方减去一个blockinterval以改变第一次开始划分的时间。之后间隔为period，即我们设定的batchinterval
    //(math.floor(clock.getTimeMillis().toDouble / period) + 1).toLong * period
    //(math.floor(clock.getTimeMillis().toDouble / period) + 1).toLong * period - 100
    (math.floor(clock.getTimeMillis().toDouble / period) + 1).toLong * period - blockInterval * 2
  }
   ...
   */
  def start(): Long = {
    if (name == "BlockGenerator") {
      start(getAstraeaTime())
    } else {
      start(getStartTime())
    }
    /*
    name match {
      case "BlockGenerator" => start(getAstraeaTime())
      case _ => start(getStartTime())
    }
