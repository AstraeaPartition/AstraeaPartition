...
class DAGScheduler(
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    /*
    if (splitkeyParBC == null) {
      splitkeyParBC = sc.broadcast(AstraeaBatchKeyInfo.globalSplitKeyPartition)
      AstraeaBatchKeyInfo.globalSplitKeyPartitionClear()
    } else {
      if (lastJobID != jobId) {
        lastJobID = jobId
        splitkeyParBC.unpersist()
        splitkeyParBC = sc.broadcast(AstraeaBatchKeyInfo.globalSplitKeyPartition)
        AstraeaBatchKeyInfo.globalSplitKeyPartitionClear()
      }
    }
     ...
     */
    //logInfo("---------Astraea info:--------- DAGScheduler splitkeyParBC.size:" + splitkeyParBC.value.size)
      ...
      stage match {
        case stage: ShuffleMapStage =>
          partitionsToCompute.map { id =>
            ...
            new ShuffleMapTask(stage.id, stage.latestInfo.attemptId, sc.getSplitKeyPar(),
              taskBinary, part, locs, stage.latestInfo.taskMetrics, properties, Option(jobId),
              Option(sc.applicationId), sc.applicationAttemptId)
          }
